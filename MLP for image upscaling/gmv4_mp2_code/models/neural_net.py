...
class NeuralNetwork:
    ...
    def update(
        ...
        if self.opt == 'SGD':
            ...
            for i in range(1,self.num_layers+1) :
                self.params["W"+str(i)] -= lr*R*self.params["W"+str(i)] + lr*np.mean(self.gradients["dW"+str(i)],axis=0)
                self.params["b"+str(i)] -= lr*R*self.params["b"+str(i)] + lr*np.mean(self.gradients["db"+str(i)],axis=0)
        elif self.opt == 'Adam':
